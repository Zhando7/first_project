...
<div class="container">
	<div class="row col-sm-12">
		...
		<a href="/expert/indexevapr" class="btn btn-primary" title="Вернуться на предыдущую страницу"><i class="fa fa-reply-all"></i> Вернуться</a>
	</div>
	<div class="row col-sm-12" style="margin-top:20px;">
		<?php foreach($partaker as $itempr): ?>
		<div class="table-responsive">
		<table class="table table-hover table-bordered">
			<thead>
				<tr class="text-center">
					<?php if($namepr = Users::getPr($itempr->IdPartaker)):?>
						<td class="danger" colspan="5">
							<h4>
								<a target="_blank" href="/expert/infopr/<?= $itempr->IdPartaker; ?>" title="Информация об участнике"><i class="fa fa-info-circle"></i></a>
								<?= ''.$namepr->Surname.' '.$namepr->Name.' '.$namepr->Patronymic; ?>
							</h4>
						</td>
					<?php else: ?>
			...
			<tbody>
				<?php foreach($model as $index => $item): ?>
					<?php if($itempr->IdPartaker == $item->IdPartaker): ?>
					<tr class="text-center">
